...
use super::{CustomPointLight, CustomPointLightRaw};

/// the names of pbr uniforms
pub mod uniform {
    pub const LIGHTS: &str = "CustomLights";
}
...
pub fn lights_node_system(
    ...
    query: Query<(&CustomPointLight, &GlobalTransform)>,
) {
    let state = &mut state;
    ...
    let size = std::mem::size_of::<CustomPointLightRaw>();
    let light_count_size = std::mem::size_of::<LightCount>() + ambient_light_size;
    let light_array_size = size * light_count;
    let light_array_max_size = size * 256; //state.max_lights;
    let current_light_uniform_size = light_array_size + light_count_size;
    let max_light_uniform_size = light_array_max_size + light_count_size;
        ...
        &mut |data, _renderer| {
                ...
                .zip(data[light_count_size..current_light_uniform_size].chunks_exact_mut(size))
            {
                slot.copy_from_slice(
                    CustomPointLightRaw::from(&light, &global_transform).as_bytes(),
                );
            }
        },
    );
